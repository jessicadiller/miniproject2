...
float calc_torque(){
    ...
    vout.b[1] = vout.b[1]&ANALOG_MASK; //masks last 6 digits
...
uint16_t wall_control(int position){
    // input current angle, ouput desired torque (PWM) 
    float torque = calc_torque(); 
    int ideal, pwm; 
    WORD duty;
    int threshold = get_wall_threshold(); // in degrees
    pwm = 0;
    /*duty.w = pin_read(&D[7]); 
    if (duty.w == 0x0){
      duty.w = pin_read(&D[8]);
